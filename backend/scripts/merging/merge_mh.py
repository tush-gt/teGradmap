"""
GradMap — MH Quota Merge Pipeline
===================================
Merges all normalized MH quota CSVs into a single master dataset.

Reads from  : normalized/mh/*.csv
Writes to   : merged/mh/master_mh_dataset.csv

Pipeline:
  1. Discover & load all normalized CSVs
  2. Concatenate into single DataFrame
  3. Add dataset_type column ("MH")
...
INPUT_DIR = BASE_DIR / "normalized" / "mh"
OUTPUT_DIR = BASE_DIR / "merged" / "mh"
OUTPUT_FILE = OUTPUT_DIR / "master_mh_dataset.csv"
...
log = logging.getLogger("merge_mh")
...
def main() -> None:
    log.info("═" * 50)
    log.info("GradMap — MH Quota Merge Pipeline")
    log.info("═" * 50)
